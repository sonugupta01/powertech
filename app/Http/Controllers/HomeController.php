...
class HomeController extends Controller
{
    ...
    public function getDistrict(Request $request)
    {
        $post = $request->all();
        $state = $request->state;
        $districts = DB::table('districts')->where('state_id', $state)->orderBy('district_name', 'ASC')->get();
    ...
    public function getAdvisors(Request $request)
    {
        ...
        $advisors = DB::table('advisors as a')
            ...
            ->select('a.id', 'a.name', 'a.department', 'd.name as department_name')->where('a.dealer_id', $dealer)
            ->where('a.status', 1)
            ->get();
    ...
    public function cronJob()
    {
        ...
        $dealers = DB::table('users_email')->select('user_id', DB::raw('group_concat(email) as email'))->groupBy('user_id')->orderBy('user_id')->where('user_id', '!=', 58)->get();
        // echo "<pre>";
        // dd($dealers);
        //die;
        foreach ($dealers as $key => $value1) {
            $data = DB::table('jobs')
                ->select(DB::raw('group_concat(id) as job_id, SUM(customer_price) as vas_customer_price, SUM(hvt_value) as hvt_customer_price,  SUM(incentive) as vas_incentive, advisor_id'))
                ->where('dealer_id', '=', $value1->user_id)
                ->whereDate('job_date', '>=', $firstDate)
                ->whereDate('job_date', '<=', $currentDate)
                ->where('delete_job', 1)
                ->groupBy('advisor_id')
                ->get();
            $advisor = array();
            ...
            if (count($data) > 0) {
                foreach ($data as $value) {
                    $today_data = DB::table('jobs')
                        ->select(DB::raw('group_concat(id) as job_id, SUM(customer_price) as today_vas_customer_price, SUM(hvt_value) as today_hvt_customer_price,  SUM(incentive) as today_vas_incentive, advisor_id'))
                    ...
                    foreach ($d as  $val) {
                        $treat = DB::table('jobs')->select('treatments')->where('id', $val)->where('delete_job', 1)->first();
                    ...
                    @$total_service = DB::table('jobs_by_date')
                        ...
                        ->where('dealer_id', '=', $value1->user_id)
                        ->whereDate('job_added_date', '>=', $firstDate)
                        ->whereDate('job_added_date', '<=', $currentDate)
                        ->first();
                    ...
                    @$today_service = DB::table('jobs_by_date')
                        ->select(DB::raw('SUM(total_jobs) as total'))
                        ->where('dealer_id', '=', $value1->user_id)
                        ->whereDate('job_added_date', $currentDate)
                        ->first();

                    @$today_jobs = DB::table('jobs')
                        ->select(DB::raw('SUM(vas_value) as vas_value,SUM(vas_total) as vas_total, SUM(hvt_value) as hvt_value,SUM(hvt_total) as hvt_total'))
                        ->where('dealer_id', '=', $value1->user_id)
                        ->whereDate('job_date', $currentDate)
                        ->where('delete_job', 1)
                        ->first();
            ...
            $message = array('advisors' => $advisor, 'total_job_array' => @$total_job_array);
    ...
    {
        sendgrid();
        $arr = array('result' => sendgrid(), 'status_code' => 204);
    ...
    public function send_mail_of_late_commer($id)
    {
        ...
        //   dd($data);
        if ($interval == 3) {
            $user = DB::table('users')->whereIn('users.role', [3, 4, 5])->get();
