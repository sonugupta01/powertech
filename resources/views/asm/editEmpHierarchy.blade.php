...
<div class="content-wrapper">
  ...
  <section class="content-header">
    ...
    <ol class="breadcrumb">
      ...
      {{-- <li><a href="{{url('/asm/emp_hierarchy')}}"><i class="fa fa-users"></i> Attendance Shifting</a></li> --}}
      <li><a href="{{url('/asm/staff_management')}}"><i class="fa fa-users"></i> Staff Management</a>
      ...
      <div class="col-xs-12">
        <div class="box">
          ...
          <div class="box-body">
            ...
            @endif
            <form role="form" id="EmpHierarchyForm" method="POST" action="{{url('/asm/updateEmpHierarchy')}}" enctype="multipart/form-data" onsubmit="return validateForm()">
...
$(document).ready(function(){
  $("#del_id").on('change',function(){
    ...
    $.ajax({
        url: "{{ url('asm/getDealerPermission') }}/"+ user_id + "/" + del_id,
        method: 'GET',
...
$(document).ready(function(){
  $("#authority").on('change',function(){
    ...
    var user_id = $('#userId').val();
    $.ajax({
        url: "{{ url('asm/getauthority') }}/"+ user_id + "/" + auth_id,
        method: 'GET',
        success: function(data) {
          if (data.html) {
            alert(data.html);
...
$(document).ready(function(){
  $("#del_id").on('change',function(){
    ...
    $.ajax({
      url: "{{ url('asm/getdealerauthority') }}",
...
$(document).ready(function(){
  $("#dealer_authid").on('change',function(){
    ...
    var user_id = $('#userId').val();
    $.ajax({
        url: "{{ url('asm/getreportingpermission') }}/"+ user_id + "/" + del_id + "/" + del_authid,
        method: 'GET',
        success: function(data) {
          if (data.html) {
            alert(data.html);
...
$(document).ready(function(){
  $("#reporting_level").on('change',function(){
    ...
    $.ajax({
      url: "{{ url('asm/getreportingauthority') }}",
      headers: {'X-CSRF-TOKEN': token},
      method: 'POST',
      data: data,
      datatype: 'JSON',
