...
<div class="content-wrapper">
  ...
  <section class="content-header">
    ...
    <ol class="breadcrumb">
      ...
      {{-- <li><a href="{{url('/admin/emp_hierarchy')}}"><i class="fa fa-users"></i> Attendance Shifting</a></li> --}}
      <li><a href="{{url('/admin/staff_management')}}"><i class="fa fa-users"></i> Staff Management</a>
      ...
      <div class="col-xs-12">
        <div class="box">
          ...
          <div class="box-body">
            ...
            @endif
            <form role="form" id="EmpHierarchyForm" method="POST" action="{{url('/admin/updateEmpHierarchy')}}" enctype="multipart/form-data" onsubmit="return validateForm()">
                  ...
                  <div class="row">
                    <div class="col-md-6">
                      <div class="form-group{{ $errors->has('fdate') ? ' has-error' : '' }}">
                        ...
                        <input type="text" class="form-control required" value="{{ old('fdate', $result->from_date) }}" id="fdate" name="fdate" placeholder="Enter Date" autocomplete="off" readonly>
                        @if ($errors->has('fdate'))
                        <span class="help-block">
                    ...
                    <div class="col-md-6">
                      <div class="form-group{{ $errors->has('todate') ? ' has-error' : '' }}">
                        ...
                        <input type="text" class="form-control required" value="{{ old('todate', $result->to_date) }}" id="todate" name="todate" placeholder="Enter Date" autocomplete="off" readonly>
...
}else if($('#des_id').val() == '15') {
  $("#authoritySelector").hide();
  $("#reportingLevelSelector").hide();
}else if($('#des_id').val() == '16') {
  $("#authoritySelector").hide();
  $("#reportingLevelSelector").hide();
}else if($('#des_id').val() == '17') {
  $("#authoritySelector").hide();
  $("#reportingLevelSelector").hide();
}else if($('#des_id').val() == '19') {
  $("#authoritySelector").hide();
  $("#reportingLevelSelector").hide();
}else if($('#des_id').val() == '21') {
  $("#authoritySelector").hide();
  $("#reportingLevelSelector").hide();
}else {
...
//     $.ajax({
//         url: "{{ url('admin/getreportingauthority') }}/"+ user_id + "/" + del_id,
//         method: 'GET',
...
// var auth_id = $('#auth_hidden_id').val();
// var des_id = $('#des_id').val();
// $.ajax({
{{-- //     url: "{{ url('admin/getBydesignation') }}/" + des_id + "/" + auth_id, --}}
//     method: 'GET',
//     success: function(data) {
//       console.log(data.html);
//       $("#authority").html(data.html);
//     }
// });
...
function validateForm() {
  if ( $('#authority').val() == ''){
    ...
    if ($('#dealer_id').val() == '') {
      var y = document.forms["EmpHierarchyForm"]["dealer_id"].value;
      if (y == "" || y == null) {
